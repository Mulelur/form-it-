...
function Canvas() {
	...
	const insertPath = useMutation(
		({ storage, self, setMyPresence }) => {
			const liveLayers = storage.get('layers');
			const { pencilDraft: pencilDraft1 } = self.presence;
			if (
				pencilDraft1 == null ||
				pencilDraft1.length < 2 ||
			...
			liveLayers.set(
				id,
				new LiveObject(penPointsToPathLayer(pencilDraft1, lastUsedColor))
			);
	...
	const continueDrawing = useMutation(
		({ self, setMyPresence }, point: Point, e: React.PointerEvent) => {
			const { pencilDraft: pencilDraft2 } = self.presence;
			if (
				canvasState.mode !== CanvasMode.Pencil ||
				e.buttons !== 1 ||
				pencilDraft2 == null
			...
			setMyPresence({
				cursor: point,
				pencilDraft:
					pencilDraft2.length === 1 &&
					pencilDraft2[0][0] === point.x &&
					pencilDraft2[0][1] === point.y
						? pencilDraft2
						: [...pencilDraft2, [point.x, point.y, e.pressure]],
			});
		},
		[canvasState.mode]
	);
