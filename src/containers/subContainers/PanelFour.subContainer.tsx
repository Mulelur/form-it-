	...
	return (
		<>
			<Panel.Row>
				...
				<Panel.Row
					style={{ justifyContent: 'flex-end', left: '0', paddingLeft: '0' }}
				>
					<PanelInput
						onSubmit={() => {}}
						onChange={() => {}}
					...
					/>
					{name && onClick && (
						<Panel.SegmentedControl style={{ width: '6rem' }}>
							<Panel.SegmentsWrapper>
								<Panel.SegmentedControlSegmentBackground
									style={{
										left: `${segmentState.type === name && segmentState.state}`,
									}}
								/>
								<Panel.Segment
									onClick={() => {
			...
			{disableFour && (
				<Panel.StackRow flexDirection="column">
					...
					<Panel.Col>
						<Panel.InputWrapper>
							<Panel.Input
								onChange={(event) => topHandelChange(event, name)}
								size="x-small"
								value={Top}
							/>
							...
							<Panel.InputDivider />
							<Panel.Input
								onChange={(event) => rgithHandelChange(event, name)}
								size="x-small"
								value={Rigth}
							/>
							...
							<Panel.InputDivider />
							<Panel.Input
								onChange={(event) => bottomHandelChange(event, name)}
								size="x-small"
								value={Bottom}
							/>
							...
							<Panel.InputDivider />
							<Panel.Input
								onChange={(event) => leftHandelChange(event, name)}
								size="x-small"
								value={left}
							/>
